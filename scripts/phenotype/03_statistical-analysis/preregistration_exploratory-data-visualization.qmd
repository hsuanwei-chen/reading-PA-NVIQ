...
participants_df |> 
  tbl_summary(
    by = dataset, 
    include = c(age, sex, rs, pa, fi, nci_acc),
    label = list(
      age = "Age",
      sex = "Sex",
      rs = "Reading Skill",
...
#### Accuracy on Non-Conflicting Items (NCIs)
...
# Ridge plot for IFG
fig_ridge_acc <- participants_df |> 
  plot_ridge(
    x = "nci_acc", 
    y = "dataset", 
    fill = "dataset",
    xlim = c(0.5, 1),
    xbreaks = seq(0.5, 1, 0.1),
    fill_labels = dataset_labels
  ) +  
  labs(
    x = "Accuracy on NCIs", 
...
fig_matrix <- participants_df |>
  select(age, rs, pa, fi, nci_acc) |> 
  ggpairs(
    columnLabels = c("Age", "RS", "PA", "FI", "ACC"),
