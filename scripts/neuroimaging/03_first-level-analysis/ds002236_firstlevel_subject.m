...
function ds002236_firstlevel_subject(subjects)
%% Specify filepaths;
...
bids_folder = fullfile(root_dir, 'datasets', '3_ds002236', 'bids');
...
% Rows - 'null ''O+P+' 'O-P+' 'O+P-' 'O-P-' 'Single Symbols' 'Triple Symbols'
conditions{1}={'fixation' 'O+P+' 'O+P-' 'O-P+' 'O-P-' 'simple_perceptual' 'complex_perceptual'};
conditions{2}={'fixation' 'O+P+' 'O+P-' 'O-P+' 'O-P-' 'simple_perceptual' 'complex_perceptual'};
...
% nc_fix = [-1/2 0.5/2, 0, 0, 0.5/2, 0, 0]
nc_fix = [-0.5 0.25 0 0 0.25 0 0];
...
try
    ...
    for j=1:length(func_dirs_rr)
        ...
        elseif events_file_exist==0
            [p,run_n]=fileparts(func_dirs_rr{j}(1:end-1));
            [q,this_subject]=fileparts(fileparts(p));
            %[~,this_subject]=fileparts(q);
            event_file=[bids_folder '/' this_subject '/func/' run_n(1:end-4) 'events.tsv'];
