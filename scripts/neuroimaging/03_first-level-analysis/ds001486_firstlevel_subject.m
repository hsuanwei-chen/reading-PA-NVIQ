...
function ds001486_firstlevel_subject(subjects)
%% Specify filepaths;
...
bids_folder = fullfile(root_dir, 'datasets', '6_ds001486', 'bids');
...
% Rows - 'O+P+' 'O+P-' 'O-P+' 'O-P-' 'Control, blue box judgement' 'Perceptual'
...
try
    ...
    for j=1:length(func_dirs_rr)
        ...
        cond=cond(1:6, :); % Select first 6 rows, last row is NA.
        [~,len]=size(cond);
        ...
        for k=1:size(cond,1)
            % Event_data.trial_type is a character array
