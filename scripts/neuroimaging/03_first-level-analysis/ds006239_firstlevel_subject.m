...
function ds006239_firstlevel_subject(subjects)
%% Specify filepaths;
...
CCN.file = 'vs6_wsub*bold.nii';
CCN.rpfile = 'rp_';
 
%% Specify Task Conditions
% Define your task conditions, each run is a cell
% Rows - 'O+P+' 'O-P+' 'O+P-' 'O-P-' 'PercY' 'PercN' 'FixY' 'FixN'
conditions{1}={'O+P+' 'O-P+' 'O+P-' 'O-P-' 'PercY' 'PercN' 'FixY' 'FixN'};
conditions{2}={'O+P+' 'O-P+' 'O+P-' 'O-P-' 'PercY' 'PercN' 'FixY' 'FixN'};
...
% nc_fix = [0.5/2, 0, 0, 0.5/2, 0, 0, -0.5/2, -0.5/2]
nc_fix = [0.25 0 0 0.25 0 0 -0.25 -0.25];
