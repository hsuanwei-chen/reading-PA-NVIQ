...
idx = strcmp(subj_tbl.dataset, 'ds006239') & strcmp(subj_tbl.exclude, 'NA');
...
CCN.funcf1 = 'sub*_*_task-ReadPhon*_run-1_bold.ni*';
CCN.funcf2 = 'sub*_*_task-ReadPhon*_run-2_bold.ni*';
...
multiple_run2 = 'multiple_T1w.txt';
if exist(multiple_run2, 'file')
    delete(multiple_run2);
end
fid2 = fopen([proc_dir '/' multiple_run2], 'w');
...
% List of subjects with multiple run-1s
multiple_run1 = 'multiple_task-run01.txt';
if exist(multiple_run1, 'file')
    delete(multiple_run1);
end
fid6 = fopen([proc_dir '/' multiple_run1], 'w');

% List of subjects with multiple run-2s
multiple_run2 = 'multiple_task-run02.txt';
if exist(multiple_run2, 'file')
    delete(multiple_run2);
end
fid7 = fopen([proc_dir '/' multiple_run2], 'w');
...
for i = 1:length(subjects)
    ...
    if ~isempty(expand_path([old_dir '/func/' '[funcf1]'])) || ...
       ~isempty(expand_path([old_dir '/func/' '[funcf2]']))
       ...
       % Copy functional data and maintain group permissions
        ...
        % Check for number of functional run-1 scans
        source{1} = expand_path([old_dir '/func/[funcf1]']);
        if length(source{1}) > 1
            disp('!!! Multiple Run-01s found !!!')
            fprintf(fid6, '%s\n',  subjects{i});
        end
        
        % Check for number of functional run-2 scans
        source{2} = expand_path([old_dir '/func/[funcf2]']);
         if length(source{2}) > 1
             disp('!!! Multiple Run-02s found !!!')
            fprintf(fid7, '%s\n',  subjects{i});
        end
